      ...
      persistenceDef // {
        ...
        lh.services.traefik.dynamicConfig = lib.mkIf config.lh.services.grafana.traefikIntegration.enable {
          http.routers.grafana = {
            ...
            service = "grafana";
            tls = {
              inherit (config.lh.services.grafana.traefikIntegration) certResolver;
            };
          };
          http.services.grafana.loadBalancer.servers = [{ url = "unix+http:/run/grafana/grafana.sock"; }];
        };

        systemd.services.traefik = lib.mkIf config.lh.services.grafana.traefikIntegration.enable {
          serviceConfig = {
