...
in
{
  ...
  config =
    ...
    users.users.traefik = {
      isSystemUser = true;
      group = "traefik";
      extraGroups = optionals config.lh.services.grafana.enable [ "grafana" ];
    };

    users.groups.traefik = { gid = 9443; };
    ...
    systemd.services.traefik = {
      serviceConfig = {
        ...
        SystemCallFilter =
          [ "~@privileged" "~@cpu-emulation" "~@debug" "~@mount" "~@obsolete" ];
        SystemCallErrorNumber = "EPERM";
        RestrictAddressFamilies = [ "AF_UNIX" "AF_INET" "AF_INET6" ];
        AmbientCapabilities = [ ];
        UMask = "0077";

        RootDirectory = "/var/run/jails/traefik";
