...
in
{
  ...
  home-manager.users.leon = { pkgs, lib, osConfig, ... }:
    let hostname = osConfig.networking.hostName or "";
    in {
      nixpkgs.config.allowUnfree = true;
      nixpkgs.overlays = [
        nix4vscode.overlays.default
        (_self: _super: { inherit (pkgsUnstable) vscode; })
      ];
  ...
  users.users.leon = {
    ...
    uid = 1000;
    home = "/home/leon";
    extraGroups = [ "users" "networkmanager" "wheel" "dialout" ];
  };
